...
`src/lib/env.ts` validates these at startup, so a missing value fails with a
clear message rather than an obscure runtime error.
...
docs/                  security notes, launch checklist
