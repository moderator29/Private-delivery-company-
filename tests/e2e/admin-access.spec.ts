...
test.describe("unauthorised access to the admin area", () => {
  ...
  for (const path of guarded) {
    test(`redirects a signed-out visitor from ${path} to the sign-in page`, async ({
      page,
    }) => {
      await page.goto(path);

      await expect(page).toHaveURL(
        new RegExp(`/admin/login\\?next=${encodeURIComponent(path)}$`),
      );
      await expect(
  ...
  test("never renders operational data on the way to the sign-in page", async ({
    page,
  }) => {
