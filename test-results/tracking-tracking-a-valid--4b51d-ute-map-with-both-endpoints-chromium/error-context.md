...
- Name: tracking.spec.ts >> tracking a valid shipment >> shows the route map with both endpoints
- Location: tests/e2e/tracking.spec.ts:24:7
...
Locator: getByRole('img', { name: /Route map from .* to .*/ })
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for getByRole('img', { name: /Route map from .* to .*/ })
