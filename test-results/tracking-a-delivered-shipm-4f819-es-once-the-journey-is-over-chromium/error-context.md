...
- Name: tracking.spec.ts >> a delivered shipment >> stops projecting milestones once the journey is over
- Location: tests/e2e/tracking.spec.ts:113:7
...
Locator:  locator('ol').filter({ hasText: 'Shipment Information Received' }).locator('> li')
Expected: 6
Received: 0
Timeout:  5000ms
...
Call log:
  ...
  - waiting for locator('ol').filter({ hasText: 'Shipment Information Received' }).locator('> li')
...
```yaml
- 'heading "Application error: a client-side exception has occurred while loading 127.0.0.1 (see the browser console for more information)." [level=2] [ref=e4]'
```
